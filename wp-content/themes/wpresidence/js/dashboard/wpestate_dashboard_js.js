...
/**
 * Toggle package descriptions and automatically select first package
 */
function initializePackagesToggle() {
    // Bind click event to the open packages button
    jQuery('#open_packages').on('click', function(event) {
        // Prevent default action if it's a link
        event.preventDefault();
        
        // Toggle visibility of package description rows with slide animation
        jQuery('.pack_description_row').slideToggle();
        
        // Toggle the 'open_pack_on' class on the clicked button for styling
        jQuery(this).toggleClass('open_pack_on');
        
        // Automatically trigger click on the buy button of the first package
        jQuery('.pack-listing:first').find('.buypackage').trigger('click');
        event.stopPropagation();
    });
}
